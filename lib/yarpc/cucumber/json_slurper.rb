...
module YARPC
  module Cucumber
    ...
    class JSONSlurper
      ...
      def run
        YARPC.start_launch('', get_time)

        @json.each do |feature|
          current_feature = YARPC::TestItem.new("Feature: #{feature['name']}",
                                                :TEST,
                                                nil,
                                                get_time,
                                                feature['uri'],
          ...
          current_feature.id = YARPC.start_item(current_feature_node)
          ...
          feature['elements'].each do |element|
            ...
            YARPC.current_scenario = YARPC::TestItem.new(element_name,
                                                         type,
                                                         nil,
                                                         get_time,
                                                         ...
                                                         nil,
                                                         tags(element))
            current_scenario_node = Tree::TreeNode.new(SecureRandom.hex, YARPC.current_scenario)
            YARPC.current_scenario.id = YARPC.start_item(current_scenario_node)
            ...
            element['steps'].each do |step|
              ...
              YARPC.send_log(:passed, name, get_time)
              step['output'].each { |o| YARPC.send_log(:passed, o, get_time) } unless step['output'].nil?
              error = step['result']['error_message']
              YARPC.send_log(:failed, error, get_time) if error
              (step['embeddings'] || []).each do |embedding|
                ...
                Tempfile.open(['embedding', extension]) do |file|
                  ...
                  file.rewind
                  YARPC.send_file(:failed, file, 'Embedding', get_time)
                end
              end
              ...
              YARPC.send_log(step['result']['status'].to_sym,
            ...
            YARPC.finish_item(YARPC.current_scenario, status, get_time, forced_issue)
            YARPC.current_scenario = nil
          end

          YARPC.finish_item(current_feature, nil, get_time)
        end
        YARPC.finish_item(root, nil, get_time)
        YARPC.finish_launch(get_time)
      end
      ...
      def report_hooks(element, tag)
        (element[tag] || []).map do |hook|
          YARPC.send_log(hook['result']['status'].to_sym,
