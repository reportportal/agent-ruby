  ...
  class Settings
    ...
    def initialize
      filename = ENV.fetch("#{PREFIX}config") do
        ...
        p "Multiple configuration files found for YARPC. Using the first one: #{glob.first}" if glob.size > 1
        glob.first
      end
      ...
      keys.each do |key, is_required|
        ...
        fail "YARPC: Define environment variable '#{PREFIX}#{key}' or key #{key} in the configuration YAML file" if is_required && public_send(key).nil?
      end
    end
